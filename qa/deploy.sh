...
# which forces `up` to rebuild it. Override with QA_BUILD_TAG to force a
# rebuild without a code change.
TAG="${QA_BUILD_TAG:-$(git rev-parse --short HEAD 2>/dev/null || date +%s)}"
...
tar -cf - --exclude=node_modules --exclude=.git --exclude=.github \
  ...
  | tar -xf - -C "$STAGE"
sed "s/QA_BUILD_TAG/${TAG}/g" docker-compose.yml > "$STAGE/docker-compose.yml"
if grep -q 'QA_BUILD_TAG' "$STAGE/docker-compose.yml"; then
